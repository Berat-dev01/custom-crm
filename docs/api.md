...
> Eski sürümsüz yollar (`/api/crm/...`) 308 kalıcı yönlendirme ile `/api/crm/v1/...` adresine yönlenir. Yeni entegrasyonlarda her zaman `v1` kullanın.
>
> Makine-okunur şema: [`docs/openapi.yaml`](openapi.yaml)
...
Tokens are managed from the admin panel: **CRM > System > API Tokens** (`/admin/crm/api-tokens`). The plaintext value is displayed exactly once after creation; revoking a token immediately stops it from authenticating.

Alternatively, to issue a token from a trusted console:
